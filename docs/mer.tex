...
\begin{tikzpicture}[node distance=7em]
    ...
    \node[attribute] (research-id) at (18,3) {\key{ID}} edge (research);
    \node[attribute] (research-name) at (18,2) {Nome} edge (research);
    \node[multi attribute] (research-aggregates) at (18,1) {Agregados} edge (research);
    \node[attribute] (aggregate-id) at (21,3) {\key{ID}} edge (research-aggregates);
    \node[attribute] (aggregate-name) at (21,2) {Nome} edge (research-aggregates);
    \node[multi attribute] (aggregate-periods) at (21,1) {Períodos} edge (research-aggregates);
    \node[attribute] (aggregate-metadata) at (21,0) {Metadados} edge (research-aggregates);
    \node[attribute] (metadata-subject) at (25,2) {Assunto} edge (aggregate-metadata);
    \node[multi attribute] (metadata-variables) at (25,1) {Variáveis} edge (aggregate-metadata);
    \node[multi attribute] (metadata-classifications) at (25,0) {Classificações} edge (aggregate-metadata);

    \node[entity] (variable) at (20,-2) {VariávelIBGE};
    \node[attribute] (variable-id) at (23,-1) {\key{ID}} edge (variable);
    \node[attribute] (variable-name) at (23,-2) {Nome} edge (variable);
    ...
    \node[relationship] at (17,-1) {Resultados}
        edge (research)
        edge[total] (variable);
    \node at (18.5,-1) {$N$};
    \node at (16,0) {$1$};
